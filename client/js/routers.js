...
Router.route('/userprofile/:_id', {
    ...
    fastRender: true,
    waitOn: function(){
        return Meteor.subscribe("profile",{userId:this.params._id,
...
ChatroomsCtrl = RouteController.extend({
    ...
    data:
    {
        ...
        room: function(){
            var room = {room:Session.get("roomid"),
            ...
            if ( room.type == "public"){
                ...
                room = {name:user.profile.name,
                        _id:PrivateChat.findOne({_id:room.room})._id};
            }
            return room;
        },
        chat_users: function(){
            ...
            userList = Meteor.users.find({_id:{$in:userList} },
                                         {fields:{_id:1, status:1,
                                                  "profile.name":1,
                                                  "profile.lastname":1}});
            userList = _.map(userList.fetch(),
                             function(doc){
                                 return {_id:doc._id,
                                         name:doc.profile.name,
                                         lastname:doc.profile.lastname,
                                         fullname:doc.profile.name + " " +
        ...
        messages: function(){
            ...
            messages = Messages.find({room:messages.room}, {sort:{time: +1}}).fetch();
        ...
        group_rooms:function(){
            var user_groups = User_Room.find({user: Meteor.userId(),
                                              type: "group"},
                                             {fields:{room:1,new_messages:1}})
                    .fetch();
            var groupsArray = _.map(user_groups,function(row){
                ...
                if(group !== undefined){
                    ...
                    if (row.room == Session.get("roomid"))
                        group.active = true;
                    if(Roles.userIsInRole(Meteor.userId(), "group-manager",
                                          row.room)
                       || Roles.userIsInRole(Meteor.userId(), "owner",
                                             row.room)){
                        group.isMod = true;
                        group.request = {total:GroupRequest.find({room:row.room,
                                                                  type:1})
                                         .count()};
    ...
    data:
    {
        ...
        contacts: function(){
            ...
            /*
             ...
             // console.log(privatenotificationsArray);
             var users = new Array();
:users_relationsArray}}).fetch().forEach(function(row){
             row.notification = privatenotificationsArray[row._id];
             users.push(row);
             });*/
